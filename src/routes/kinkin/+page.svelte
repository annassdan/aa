<script>
    ...
    function copyLink(code) {
        localStorage.setItem('guest', code);
        const url = `www.anisannas.com/?ref=${code}`;
        // console.log(url);
        showAlert = false;
        showAlert = true;
        alertMessage = 'Cetak Undangan';
    ...
    function shareNew(comment) {
        const url = `https://anisannas.com/?ref=${comment.id}`;
    ...
    {#each comments as comment, i}
        <div class="flex flex-col w-[95%] max-w-[50rem] border border-gray-300 bg-gray-200 p-4 rounded-xl">
            ...
            <div class="flex flex-row justify-between mt-3">
                ...
                <a href="#" on:click={() => shareNew(comment)}
